...
class DroneDot:
    ...
    def approach_queen(self, queen_position, max_delta=0.5):
    ...
    def move_away_from_queen(self, max_delta=0.5):
    ...
    def simulate(self, queen_bee, drones):
        ...
        for step in range(self.total_steps):
            ...
            if all(drone.state == "ready_for_gold" for drone in drones):
                gold_scatter = ax.scatter(*queen_bee.get_position(), color="gold", marker="o", s=20, label="Baby Bee")
                gold_scatters.append(gold_scatter)
                print(f"\n🎉 Baby Bee Generated at: {queen_bee.get_position()}")
                for drone in drones:
...
def run_simulation():
    ...
    timestep = Step_change(interval=0.5, total_steps=60)
